...
export interface FileServiceClient {
  ...
  getSessionMessages(workspaceId: string, sessionId: string): Promise<DisplayMessage[]>;
...
export function createServerClient(baseUrl = ''): FileServiceClient {
  ...
  return {
    ...
    getSessionMessages: async (workspaceId, sessionId) => {
      const data = await request<{ messages: DisplayMessage[] }>(
        `/api/workspace/sessions/${encodeURIComponent(sessionId)}/messages?workspaceId=${encodeURIComponent(workspaceId)}`
      );
